jQuery(document).ready(function($) {
    ...
    function loadRGAACompliantCSS() {
        ...
        style.textContent = `
            ...
            #braille-non-contract-toggle:focus {
                outline: 2px solid #000;
                outline-offset: 2px;
            }
            #braille-non-contract-toggle:hover {
                background: #1565C0;
                transform: scale(1.05);
            }
            #braille-non-contract-toggle.active {
                background: #0D47A1;
            }
            #braille-non-contract-toggle .braille-icon {
                font-size: 24px;
                margin-top: -2px;
            }

            #braille-non-contract-panel {
                position: fixed;
                bottom: 80px;
                right: 20px;
                width: 90%;
            ...
            #braille-non-contract-panel[aria-hidden="false"] {
                display: block;
            ...
            .braille-non-contract-header h2 {
                margin: 0;
                font-size: 1.1em;
                color: #333;
            }
            .braille-non-contract-close {
                background: none;
                border: none;
                font-size: 20px;
                cursor: pointer;
            ...
            .braille-non-contract-close:focus {
                outline: 2px solid #000;
            }
            .braille-non-contract-close:hover {
                color: #333;
            }
            .braille-non-contract-content {
                padding: 15px;
            }
            .braille-non-contract-result {
                margin-top: 15px;
                padding-top: 15px;
                border-top: 1px solid #eee;
            }
            .braille-non-contract-original {
                font-size: 0.9em;
                color: #666;
                margin-bottom: 8px;
                word-break: break-word;
            }
            .braille-non-contract-output {
            ...
            .braille-image-toggle:focus {
                outline: 2px solid #000;
            }
            .braille-image-toggle:hover {
                background: #0D47A1;
            }
        `;
    ...
    function createAccessibleToggle() {
        ...
        brailleToggle.id = 'braille-non-contract-toggle';
        brailleToggle.setAttribute('aria-label', 'Activer le module braille non contracté');
        brailleToggle.setAttribute('aria-expanded', 'false');
        brailleToggle.setAttribute('aria-controls', 'braille-non-contract-panel');
        brailleToggle.setAttribute('tabindex', '0');
        brailleToggle.innerHTML = '<span class="braille-icon" aria-hidden="true">⠁⠃</span><span class="sr-only">Braille non contracté</span>';
    ...
    function createAccessiblePanel() {
        ...
        braillePanel.id = 'braille-non-contract-panel';
        braillePanel.setAttribute('role', 'dialog');
        braillePanel.setAttribute('aria-modal', 'true');
        braillePanel.setAttribute('aria-labelledby', 'braille-non-contract-title');
        ...
        braillePanel.innerHTML = `
            <div class="braille-non-contract-header">
                <h2 id="braille-non-contract-title">Braille Non Contracté (Grade 1)</h2>
                <button class="braille-non-contract-close" aria-label="Fermer le panneau braille non contracté" tabindex="0">
                    <span aria-hidden="true">×</span>
                </button>
            </div>
            <div class="braille-non-contract-content">
                <div class="braille-non-contract-result">
                    <div class="braille-non-contract-original" aria-label="Texte original"></div>
                    <div class="braille-non-contract-output" aria-hidden="true" role="region" aria-label="Traduction braille non contractée"></div>
                    <div class="sr-only">Traduction en braille non contracté du texte ci-dessus.</div>
                </div>
            </div>
        `;
        document.body.appendChild(braillePanel);

        braillePanel.querySelector('.braille-non-contract-close').addEventListener('click', hideBraillePanel);
        braillePanel.querySelector('.braille-non-contract-close').addEventListener('keydown', function(e) {
            if (e.key === 'Enter' || e.key === ' ') {
                e.preventDefault();
                hideBraillePanel();
            }
    ...
    function togglePanel() {
        ...
        if (isActive) {
            showBraillePanel();
            announceToScreenReader("Module braille non contracté activé. Appuyez sur Échap pour fermer.");
            if (document.activeElement) {
                handleFocusChange({ target: document.activeElement });
            }
        } else {
            hideBraillePanel();
            announceToScreenReader("Module braille non contracté désactivé.");
        }
    }
    ...
    function translateImageText(imgId, text) {
        ...
        jQuery.ajax({
            url: brailleNonContracted.ajaxurl,
            type: 'POST',
            data: {
                action: 'braille_non_contract_translate',
                text: text,
                nonce: brailleNonContracted.nonce
            },
    ...
    function activateScreenReaderMode() {
        ...
        showBraillePanel();
        announceToScreenReader("Lecteur d'écran détecté. Module braille non contracté activé automatiquement.");
    ...
    function handleFocusChange(e) {
        ...
        if (element.closest('#braille-non-contract-panel, #braille-non-contract-toggle')) {
            return;
        }
    ...
    function translateText(text) {
        ...
        document.querySelector('.braille-non-contract-original').textContent = text;
        document.querySelector('.braille-non-contract-output').innerHTML = '<div class="sr-only">Traduction en cours...</div>';
        ...
        jQuery.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    document.querySelector('.braille-non-contract-output').innerHTML = response.data.braille;
                    announceToScreenReader(`Traduction: ${text}`);
                }
            },
            error: function() {
                document.querySelector('.braille-non-contract-output').innerHTML = '<div class="sr-only">Erreur de traduction</div>';
            }
        });
    }
